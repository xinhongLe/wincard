<template>
    <div class="editable-table" :style="{ width: totalWidth + 'px' }">
        ...
        <table
            ...
            :style="
                `--themeColor: ${theme?.color}; --subThemeColor1: ${subThemeColor[0]}; --subThemeColor2: ${subThemeColor[1]}`
            "
        >
            <colgroup>
                <col
                ...
                <tr v-for="(rowCells, rowIndex) in tableCells" :key="rowIndex">
                    ...
                    >
                        <CustomTextarea
                            ...
                            :class="{
                                active:
                                    activedCell === `${rowIndex}_${colIndex}`
                            }"
                            contenteditable="plaintext-only"
                            :value="cell.text"
                            @updateValue="
                                value => handleInput(value, rowIndex, colIndex)
                            "
                            @insertExcelData="
                                value =>
                                    insertExcelData(value, rowIndex, colIndex)
                            "
                        />
                        <div
                            v-else
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const totalWidth = computed(() =>
            colSizeList.value.reduce((a, b) => a + b)
        );
        watch(
            [() => props.colWidths, () => props.width],
            () => {
                colSizeList.value = props.colWidths.map(
                    item => item * props.width
                );
            },
            { immediate: true }
        );
        ...
        const deleteRow = (rowIndex: number) => {
            ...
            for (const pos of hideCellsPos) {
                for (let i = rowIndex; i >= 0; i--) {
                    if (!isHideCell(i, pos)) {
                        _tableCells[i][pos].rowspan =
                            _tableCells[i][pos].rowspan - 1;
                        break;
                    }
                }
            }
        ...
        const deleteCol = (colIndex: number) => {
            ...
            for (const pos of hideCellsPos) {
                for (let i = colIndex; i >= 0; i--) {
                    if (!isHideCell(pos, i)) {
                        _tableCells[pos][i].colspan =
                            _tableCells[pos][i].colspan - 1;
                        break;
                    }
                }
            }
        ...
        const fillTable = (rowCount: number, colCount: number) => {
            let _tableCells: TableCell[][] = JSON.parse(
                JSON.stringify(tableCells.value)
            );
                ...
                for (let i = 0; i < maxRow; i++) {
                    for (let j = 0; j < maxCol; j++) {
                        if (tableCells.value[rowIndex + i][colIndex + j]) {
                            tableCells.value[rowIndex + i][colIndex + j].text =
                                data[i][j];
                        }
                    }
                }
        ...
        const checkCanMergeOrSplit = (rowIndex: number, colIndex: number) => {
            ...
            const canSplit =
                !isMultiSelected &&
                (targetCell.rowspan > 1 || targetCell.colspan > 1);
        ...
        const contextmenus = (el: HTMLElement): ContextmenuItem[] => {
            ...
            const { canMerge, canSplit } = checkCanMergeOrSplit(
                rowIndex,
                colIndex
            );
