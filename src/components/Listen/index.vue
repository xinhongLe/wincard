...
<script lang="ts">
import { computed, defineComponent, nextTick, onDeactivated, onMounted, onUnmounted, ref } from "vue";
import { useStore } from "@/store";
import ScaleCanvas from "@/views/Editor/Scale.vue";
...
export default defineComponent({
    ...
    setup() {
        ...
        const wordList = computed(() => store.getters.currentSlide.listenWords);
        const current = ref(-1);
        const count = ref(2);
        const time = ref(2);
