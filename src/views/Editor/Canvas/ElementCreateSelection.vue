<template>
    ...
    >
        <div
            :class="['selection', creatingElement.type, (creatingElement.type === 'shape' && !creatingElement.data.path) && 'custom']"
            v-if="(start && end) || (creatingElement.type === 'shape' && !creatingElement.data.path)"
            :style="position"
        >
            <!-- 绘制线条专用 -->
            <svg-wrapper
            ...
            <svg-wrapper
                v-if="creatingElement.type === 'shape' && !creatingElement.data.path"
                overflow="visible"
                width="100%"
                height="100%"
            >
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const createSelection = (e: MouseEvent) => {
            // 绘制自定义形状
            if (creatingElement.value && creatingElement.value.type === "shape" && !creatingElement.value.data.path) {
                let isStart = false;
                document.onmousemove = e => {
                    ...
                    const x = e.offsetX;
                    const y = e.offsetY;
                    shapeCustomPath.value[shapeCustomPath.value.length - 1] = [x, y];
                };
                ...
                document.onmouseup = e => {
                    ...
                    const x = e.offsetX;
                    const y = e.offsetY;
                    shapeCustomPath.value.push([x, y]);
                    if (shapeCustomPath.value.length === 1) shapeCustomPath.value.push([x, y]);
                };
                ...
                document.ondblclick = () => {
                    ...
                    // 处理路径数据
                    shapeCustomPath.value.forEach((point: [number, number]) => {
                        if (minX === Infinity || minX > point[0]) minX = point[0];
                        if (minY === Infinity || minY > point[1]) minY = point[1];
                        if (maxX === Infinity || maxX < point[0]) maxX = point[0];
                        if (maxY === Infinity || maxY < point[1]) maxY = point[1];
                    });

                    let path = "";
                    shapeCustomPath.value.forEach((point: [number, number], index) => {
                        path += ` ${index === 0 ? "M" : "L"} ${point[0] - minX} ${point[1] - minY}`;
                    });
                    ...
                    store.commit(MutationTypes.SET_CREATING_ELEMENT, {
                        ...
                        data: {
                            viewBox: maxX - minX,
                            customViewBox: [maxX - minX, maxY - minY],
                            custom: true,
                            path: path + " Z"
                        }
                    });
                    emit("created", {
                        start: [minX + offset.x, minY + offset.y],
                        end: [maxX + offset.x, maxY + offset.y]
                    });
                    shapeCustomPath.value = [];
        ...
        // 绘制自定义形状的路径相关数据（仅当绘制元素类型为自定义形状时使用）
        const shapeCustomPath = ref<[number, number][]>([]);
        const shapePath = computed(() => {
            ...
            let path = "";
            shapeCustomPath.value.forEach((point: [number, number], index) => {
                path += ` ${index === 0 ? "M" : "L"} ${point[0]} ${point[1]}`;
            });
