    ...
    <div class="chart-style-panel">
        ...
        <template v-if="handleElement.chartType === 'line'">
            <div class="row">
                ...
                <a-checkbox
                    @change="
                        e => updateOptions({ showLine: !e.target.checked })
                    "
                    :checked="!showLine"
                    style="flex: 1;"
                    >散点图样式</a-checkbox
            ...
            <div class="row">
                <a-checkbox
                    @change="
                        e => updateOptions({ lineSmooth: e.target.checked })
                    "
                    :checked="lineSmooth"
                    >使用平滑曲线</a-checkbox
                >
        ...
        <div class="row" v-if="handleElement.chartType === 'bar'">
            <a-checkbox
                @change="
                    e => updateOptions({ horizontalBars: e.target.checked })
                "
        ...
        <div class="row">
            ...
            <a-select
                style="flex: 3;"
                :value="legend"
                @change="value => updateLegend(value)"
            >
...
export default defineComponent({
    ...
    setup() {
        ...
        const handleElement = computed<PPTChartElement>(
        ...
        const updateTheme = (color: string, index: number) => {
            const props = {
                themeColor: themeColor.value.map((c, i) =>
                    i === index ? color : c
                )
            };
            store.commit(MutationTypes.UPDATE_ELEMENT, {
