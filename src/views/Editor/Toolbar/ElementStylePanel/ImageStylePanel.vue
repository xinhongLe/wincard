<template>
    <div class="image-style-panel">
        ...
        <a-button-group class="row">
            ...
            >
            <a-popover trigger="click" v-model:visible="clipPanelVisible">
                <template #content>
                    <div class="clip">
                        ...
                        <template
                            v-for="type in ratioClipOptions"
                            :key="type.label"
                        >
                ...
                <a-button class="no-padding" style="flex: 1;"
                    ><IconDown
                /></a-button>
            </a-popover>
        </a-button-group>
        ...
        <a-popover trigger="click">
            ...
            <a-button class="full-width-btn"
                ><IconColorFilter class="btn-icon" /> 设置滤镜</a-button
            >
        </a-popover>
        ...
        >
        <a-button class="full-width-btn" @click="setBackgroundImage()"
            ><IconTheme class="btn-icon" /> 设为背景</a-button
        >
    </div>
</template>
...
export default defineComponent({
    ...
    setup() {
        ...
        const handleElement = computed<PPTImageElement>(
                ...
                if (filters) {
                    filterOptions.value = defaultFilters.map(item => {
                        if (filters[item.key] !== undefined) {
                            return {
                                ...item,
                                value: parseInt(filters[item.key])
                            };
                        }
                        return item;
                    });
        ...
        const clipImage = () => {
            store.commit(
                MutationTypes.SET_CLIPING_IMAGE_ELEMENT_ID,
        ...
        const getImageElementDataBeforeClip = () => {
            ...
            const originWidth =
                imgWidth /
                ((originClipRange[1][0] - originClipRange[0][0]) / 100);
            const originHeight =
                imgHeight /
                ((originClipRange[1][1] - originClipRange[0][1]) / 100);
            const originLeft =
                imgLeft - originWidth * (originClipRange[0][0] / 100);
            const originTop =
                imgTop - originHeight * (originClipRange[0][1] / 100);
