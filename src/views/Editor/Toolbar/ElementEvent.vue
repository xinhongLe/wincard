<template>
    <div class="el-event-box">
        <div class="tip"><IconClick />该元件点击触发 请选择触发对象</div>
        <a-form
            :model="formState"
            :label-col="{ span: 7 }"
            :wrapper-col="{ span: 17 }"
        ...
        <a-button type="primary" block @click="addAction">新增</a-button>
        ...
        >
            ...
            >
                <template v-slot:title>
                    <div>{{ item.inAni ? "进入动画:" + animationTypes[item.inAni] : "" }}</div>
                    <div>{{ item.outAni ? "退出动画:" + animationTypes[item.outAni] : "" }}</div>
...
export default defineComponent({
    setup() {
        ...
        const addAction = () => {
            ...
            const _actions: PPTElementAction[] =
                handleElement.value.actions || [];
