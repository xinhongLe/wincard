...
export default (
    ...
    let elOriginRotate: number;
    let elOriginWidth: number;
    let elOriginHeight: number;
    let originActiveElementList: PPTElement[];
    const sorptionRange = 5;
    const onMove = (currentPageX: number, currentPageY: number, element: PPTElement) => {
        // 开始移动
        ...
        if (
            ...
            if (elOriginRotate) {
                ...
                targetMaxX =
                        targetLeft + Math.max(element.start[0], element.end[0]);
                targetMinY = targetTop;
                targetMaxY =
                        targetTop + Math.max(element.start[1], element.end[1]);
            } else {
                targetMinX = targetLeft;
            ...
            for (let i = 0; i < originActiveElementList.length; i++) {
                ...
                if ("rotate" in element && element.rotate) {
                    ...
                    topValues.push(top);
                    rightValues.push(
                        left + Math.max(element.start[0], element.end[0])
                    );
                    bottomValues.push(
                        top + Math.max(element.start[1], element.end[1])
                    );
                } else {
                    leftValues.push(left);
                    topValues.push(top);
    ...
    const onStart = (element: PPTElement) => {
        ...
        originActiveElementList = originElementList.filter(el =>
            activeElementIdList.value.includes(el.id)
        );
        elOriginLeft = element.left;
        elOriginTop = element.top;
        elOriginWidth = element.width;
        elOriginHeight = "height" in element && element.height ? element.height : 0;
        elOriginRotate = "rotate" in element && element.rotate ? element.rotate : 0;
        ...
        const edgeTopLine: AlignLine = { value: 0, range: [0, edgeWidth.value] };
        const edgeBottomLine: AlignLine = { value: edgeHeight.value, range: [0, edgeWidth.value] };
        const edgeHorizontalCenterLine: AlignLine = { value: edgeHeight.value / 2, range: [0, edgeWidth.value] };
        const edgeLeftLine: AlignLine = { value: 0, range: [0, edgeHeight.value] };
        const edgeRightLine: AlignLine = { value: edgeWidth.value, range: [0, edgeHeight.value] };
        const edgeVerticalCenterLine: AlignLine = { value: edgeWidth.value / 2, range: [0, edgeHeight.value] };
    ...
    const mouseDragElement = (e: MouseEvent, element: PPTElement) => {
        startPageX = e.pageX;
        startPageY = e.pageY;
        onStart(element);
        document.onmousemove = e => {
            ...
            onMove(currentPageX, currentPageY, element);
        };
    ...
    const touchDragElement = (e: TouchEvent, element: PPTElement) => {
